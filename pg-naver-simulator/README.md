...
v1 사용자 표기는 **PG2**, provider ID는 **NAVER**입니다. 기존 모듈을 그대로 연결합니다. [PG 연결·확장 요구사항](../docs/requirements/03-payment-pg.md)의 인증/조회/웹훅/대사는 목표이며 아래 현재 구현과 구분합니다.

실제 네이버 결제사 연동이 불가한 환경에서 사용하는 네이버 역할의 독립 대체 PG입니다. 전용 PostgreSQL(`pgnaver` DB, 호스트 5437)에 결제와 거래 이력을 저장하며 Flyway V1로 초기화합니다. 이 모듈은 `api`(MVC Controller) → `application`(트랜잭션·상태 전이·멱등성) → `infrastructure`(JPA Entity·Repository) 구조를 사용합니다. Controller는 HTTP 변환만 수행하고 SQL을 직접 실행하지 않습니다.
...
오류와 결제 응답 DTO는 `pg-simulator-common`의 공통 API 계약을 사용하며, 서버 내부에서는 `ErrorCode → PgNaverSimulatorException → RestControllerAdvice → 오류 응답` 순서로 변환합니다.
...
주요 코드: `NAVER_PG_001` 요청값 오류, `NAVER_PG_002` 멱등 키 누락, `NAVER_PG_003` 결제 없음, `NAVER_PG_004` 가맹점 거래 번호 충돌, `NAVER_PG_005` 상태 충돌, `NAVER_PG_006` 환불 금액 오류, `NAVER_PG_007` 환불 멱등성 충돌, `NAVER_PG_999` 내부 오류.
...
주요 테이블은 `pgnaver.pg_payment`과 `pgnaver.pg_transaction`입니다. JPA의 DDL 자동 생성은 비활성화하고 Flyway 스키마를 그대로 사용합니다. 컨테이너를 재시작해도 Docker volume이 유지되는 동안 데이터와 승인·취소·환불 이력이 보존됩니다. `local`·`dev`·`stage`·`prod` 프로필을 제공하지만, 어떤 환경에서도 실제 금전 이동을 만들지 않는 simulator입니다.
