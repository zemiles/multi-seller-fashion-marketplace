...
import multi.com.marketplace.pgnaversimulator.infrastructure.*;
...
class PgConcurrencyTests {
    @Autowired PgNaverSimulatorService service;
    ...
    void simultaneousApprovalsReturnOneCommittedPaymentAndOneLedgerEntry() throws Exception {
        ...
        var values = race(8, () -> service.approve(new PgNaverSimulatorService.ApproveCommand(
        ...
        assertEquals(1L, jdbc.queryForObject(
                "select count(*) from pgnaver.pg_transaction where payment_id = ?", Long.class, id));
    }

    @Test
    void concurrentRefundRetriesHaveOnlyOneFinancialEffect() throws Exception {
        String merchant = "refund-race-" + UUID.randomUUID();
        var payment = service.approve(new PgNaverSimulatorService.ApproveCommand(merchant, "KRW", 1000, merchant));
        race(8, () -> service.refund(payment.paymentId(), new PgNaverSimulatorService.RefundCommand(700, "same")));
        assertEquals(700, service.findPayment(payment.paymentId()).refundedAmount());
        assertEquals(1L, jdbc.queryForObject(
                "select count(*) from pgnaver.pg_transaction where payment_id = ? and transaction_type = 'REFUND'",
    ...
    void competingDifferentRefundsCannotExceedPaymentAmount() throws Exception {
        ...
        var results = race(2, () -> {
            try {
                service.refund(payment.paymentId(), new PgNaverSimulatorService.RefundCommand(700, UUID.randomUUID().toString()));
                return true;
            } catch (PgNaverSimulatorException ex) {
                assertEquals(PgNaverSimulatorErrorCode.REFUND_AMOUNT_INVALID, ex.errorCode());
                return false;
            }
        });
