...
import multi.com.marketplace.pgnaversimulator.application.PgNaverSimulatorErrorCode;
import multi.com.marketplace.pgnaversimulator.application.PgNaverSimulatorException;
...
public class PgNaverSimulatorExceptionHandler {

    @ExceptionHandler(PgNaverSimulatorException.class)
    ResponseEntity<PgErrorResponse> handle(PgNaverSimulatorException exception,
    ...
    ResponseEntity<PgErrorResponse> handleInvalidRequest(Exception exception,
                                                                        HttpServletRequest request) {
    ...
    ResponseEntity<PgErrorResponse> handleUnexpected(Exception exception,
                                                                    HttpServletRequest request) {
        return error(HttpErrorCode.INTERNAL_SERVER_ERROR, request);
    }

    private static ResponseEntity<PgErrorResponse> error(PgNaverSimulatorErrorCode errorCode,
                                                                         HttpServletRequest request) {
        return ResponseEntity.status(errorCode.status())
                .body(new PgErrorResponse(errorCode.code(), errorCode.message(), Instant.now(),
                        request.getRequestURI()));
    }

    private static ResponseEntity<PgErrorResponse> error(HttpErrorCode errorCode,
                                                          HttpServletRequest request) {
        return ResponseEntity.status(errorCode.status())
                .body(new PgErrorResponse(errorCode.code(), errorCode.message(), Instant.now(),
                        request.getRequestURI()));
