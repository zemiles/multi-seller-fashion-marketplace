...
--  DIS-02; externally owned product/revision IDs; no Commerce DB read.
...
-- DIS-01; partial unique status ACTIVE; verified atomic generation switch.
CREATE TABLE marketplace.search_projection_generation (
...
-- DIS-01; all product/seller/revision IDs external, not FKs; query indexes on category/price/time+product_id.
CREATE TABLE marketplace.search_product_projection (
    generation_id uuid NOT NULL,
    product_id uuid NOT NULL,
...
ALTER TABLE marketplace.search_product_projection ADD CONSTRAINT search_product_projection_generation_id_planned_fk FOREIGN KEY (generation_id) REFERENCES marketplace.search_projection_generation (generation_id);
