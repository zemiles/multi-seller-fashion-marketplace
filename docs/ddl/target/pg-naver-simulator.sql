-- pg-naver-simulator: empty isolated database schema snapshot.
...
BEGIN;
CREATE SCHEMA IF NOT EXISTS pgnaver;

CREATE TABLE pgnaver.pg_payment (
...
CREATE TABLE pgnaver.pg_transaction (
...
CREATE TABLE pgnaver.pg_webhook_outbox (
...
CREATE TABLE pgnaver.pg_fault_scenario (
...
ALTER TABLE pgnaver.pg_transaction ADD CONSTRAINT pg_transaction_payment_id_fkey FOREIGN KEY (payment_id) REFERENCES pgnaver.pg_payment (payment_id);
ALTER TABLE pgnaver.pg_webhook_outbox ADD CONSTRAINT pg_webhook_outbox_transaction_id_planned_fk FOREIGN KEY (transaction_id) REFERENCES pgnaver.pg_transaction (transaction_id);
CREATE INDEX pg_transaction_payment_idx ON pgnaver.pg_transaction(payment_id, created_at DESC);
CREATE INDEX ix_pg_transaction_cursor_v1 ON pgnaver.pg_transaction (created_at, transaction_id);
CREATE INDEX ix_pg_webhook_due_v1 ON pgnaver.pg_webhook_outbox (status, next_retry_at, event_id);
COMMIT;
