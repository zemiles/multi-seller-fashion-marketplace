...
--  ORD-05; expiry never overrides financial hold.
CREATE TABLE marketplace.inventory_reservation (
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    recovery_hold boolean NOT NULL,
...
--  ORD-04; payment UNKNOWN is not order_status.
CREATE TABLE marketplace.orders (
    ...
    payment_snapshot_hash text NULL,
...
--  OPS-01; EXPIRED already exists; map DB PENDING to HTTP APPROVAL_PENDING. Do not overwrite prior migration.
...
CREATE TABLE marketplace.admin_recovery_code (
    admin_user_id uuid NOT NULL,
...
CREATE TABLE marketplace.verification_token (
...
CREATE TABLE marketplace.seller_invitation (
...
CREATE TABLE marketplace.command_idempotency (
...
CREATE TABLE marketplace.claim_quote (
...
CREATE TABLE marketplace.payment_dispatch_intent (
    order_id uuid NOT NULL,
...
CREATE TABLE marketplace.order_item_unit (
    order_item_id uuid NOT NULL,
    ...
    status text NOT NULL,
    fulfillment_generation integer NOT NULL,
    version bigint NOT NULL,
    ...
    PRIMARY KEY (order_item_id, unit_ordinal)
);

-- FUL-01; same order unit lock enforces only one active outbound per generation.
CREATE TABLE marketplace.shipment_item_unit (
    shipment_item_id uuid NOT NULL,
    order_item_id uuid NOT NULL,
    unit_ordinal integer NOT NULL,
    fulfillment_generation integer NOT NULL,
    PRIMARY KEY (shipment_item_id, order_item_id, unit_ordinal)
);

-- CLM-01; immutable allocation/history.
CREATE TABLE marketplace.claim_item_unit (
    claim_item_id uuid NOT NULL,
    order_item_id uuid NOT NULL,
    unit_ordinal integer NOT NULL,
    refund_amount bigint NOT NULL,
    status text NOT NULL,
    PRIMARY KEY (claim_item_id, order_item_id, unit_ordinal)
);

-- CLM-01; unique live claim per unit; terminal history remains in claim_item_unit.
CREATE TABLE marketplace.active_unit_claim_guard (
    order_item_id uuid NOT NULL,
    unit_ordinal integer NOT NULL,
    claim_item_id uuid NOT NULL,
    PRIMARY KEY (order_item_id, unit_ordinal)
...
-- FUL-02; local confirmation event writer; no second income after exchange.
CREATE TABLE marketplace.purchase_confirmation_unit (
    order_item_id uuid NOT NULL,
    unit_ordinal integer NOT NULL,
...
-- SET-03; barrier and fence and new hold share seller lock. Stream watermarks use event_stream_sequence.
CREATE TABLE marketplace.seller_financial_state (
    seller_id uuid NOT NULL,
...
CREATE TABLE marketplace.payout_fence (
    fence_id uuid NOT NULL,
    seller_id uuid NOT NULL,
    settlement_id uuid NOT NULL,
    payout_attempt_id uuid NOT NULL,
    expected_seller_state_version bigint NOT NULL,
    stream_snapshot jsonb NOT NULL,
    account_snapshot_hash text NOT NULL,
    operation_hash text NULL,
    ...
    UNIQUE (dispatch_permit_id),
    UNIQUE (payout_attempt_id, fence_version)
);

-- OPS-01; 1-time execution matching approved immutable payload.
CREATE TABLE marketplace.approval_execution (
    approval_request_id uuid NOT NULL,
...
-- SEC-04; private evidence vs public image. Owner is polymorphic, checked in application.
CREATE TABLE marketplace.upload_asset (
...
ALTER TABLE marketplace.admin_recovery_code ADD CONSTRAINT admin_recovery_code_admin_user_id_planned_fk FOREIGN KEY (admin_user_id) REFERENCES marketplace.admin_user (admin_user_id);
ALTER TABLE marketplace.verification_token ADD CONSTRAINT verification_token_member_id_planned_fk FOREIGN KEY (member_id) REFERENCES marketplace.member (member_id);
ALTER TABLE marketplace.seller_invitation ADD CONSTRAINT seller_invitation_seller_id_planned_fk FOREIGN KEY (seller_id) REFERENCES marketplace.seller (seller_id);
ALTER TABLE marketplace.seller_invitation ADD CONSTRAINT seller_invitation_accepted_member_id_planned_fk FOREIGN KEY (accepted_member_id) REFERENCES marketplace.member (member_id);
ALTER TABLE marketplace.claim_quote ADD CONSTRAINT claim_quote_order_id_planned_fk FOREIGN KEY (order_id) REFERENCES marketplace.orders (order_id);
ALTER TABLE marketplace.claim_quote ADD CONSTRAINT claim_quote_member_id_planned_fk FOREIGN KEY (member_id) REFERENCES marketplace.member (member_id);
ALTER TABLE marketplace.payment_dispatch_intent ADD CONSTRAINT payment_dispatch_intent_order_id_planned_fk FOREIGN KEY (order_id) REFERENCES marketplace.orders (order_id);
ALTER TABLE marketplace.order_item_unit ADD CONSTRAINT order_item_unit_order_item_id_planned_fk FOREIGN KEY (order_item_id) REFERENCES marketplace.order_item (order_item_id);
ALTER TABLE marketplace.shipment_item_unit ADD CONSTRAINT shipment_item_unit_shipment_item_id_planned_fk FOREIGN KEY (shipment_item_id) REFERENCES marketplace.shipment_item (shipment_item_id);
ALTER TABLE marketplace.shipment_item_unit ADD CONSTRAINT shipment_item_unit_order_item_id_unit_ordinal_planned_fk FOREIGN KEY (order_item_id, unit_ordinal) REFERENCES marketplace.order_item_unit (order_item_id, unit_ordinal);
ALTER TABLE marketplace.claim_item_unit ADD CONSTRAINT claim_item_unit_claim_item_id_planned_fk FOREIGN KEY (claim_item_id) REFERENCES marketplace.claim_item (claim_item_id);
ALTER TABLE marketplace.claim_item_unit ADD CONSTRAINT claim_item_unit_order_item_id_unit_ordinal_planned_fk FOREIGN KEY (order_item_id, unit_ordinal) REFERENCES marketplace.order_item_unit (order_item_id, unit_ordinal);
ALTER TABLE marketplace.active_unit_claim_guard ADD CONSTRAINT active_unit_claim_guard_claim_item_id_order_item_id_unit_ordinal_planned_fk FOREIGN KEY (claim_item_id, order_item_id, unit_ordinal) REFERENCES marketplace.claim_item_unit (claim_item_id, order_item_id, unit_ordinal);
ALTER TABLE marketplace.purchase_confirmation_unit ADD CONSTRAINT purchase_confirmation_unit_order_item_id_unit_ordinal_planned_fk FOREIGN KEY (order_item_id, unit_ordinal) REFERENCES marketplace.order_item_unit (order_item_id, unit_ordinal);
ALTER TABLE marketplace.purchase_confirmation_unit ADD CONSTRAINT purchase_confirmation_unit_confirmation_event_id_planned_fk FOREIGN KEY (confirmation_event_id) REFERENCES marketplace.purchase_confirmation_event (confirmation_event_id);
ALTER TABLE marketplace.seller_financial_state ADD CONSTRAINT seller_financial_state_seller_id_planned_fk FOREIGN KEY (seller_id) REFERENCES marketplace.seller (seller_id);
ALTER TABLE marketplace.payout_fence ADD CONSTRAINT payout_fence_seller_id_planned_fk FOREIGN KEY (seller_id) REFERENCES marketplace.seller (seller_id);
ALTER TABLE marketplace.approval_execution ADD CONSTRAINT approval_execution_approval_request_id_planned_fk FOREIGN KEY (approval_request_id) REFERENCES marketplace.admin_approval_request (approval_request_id);
