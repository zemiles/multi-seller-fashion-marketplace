# pg-naver-simulator — v1 목표 / 미적용 데이터 사전
...
상태: MODIFIED · 실제 schema: pgnaver · 현재 누적 마이그레이션 정의
...
상태: PLANNED · 실제 schema: pgnaver · PG-03; transaction commit and webhook row atomic; timestamp/signature refreshed on retry.
...
- FK pg_webhook_outbox_transaction_id_planned_fk: (transaction_id) → pg2.pg_transaction(transaction_id)
...
상태: PLANNED · 실제 schema: pgnaver · PG-02; local/stage only; fail closed in prod profile.
