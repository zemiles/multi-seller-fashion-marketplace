# settlement-service — v1 목표 / 미적용 데이터 사전

문서용 접두사는 서비스 DB 경계를 나타냅니다. 목표 파일은 실행 DDL이 아닙니다. 원본 출처는 [ERD 안내](../README.md)를 따릅니다.

## seller_ledger_entry

상태: MODIFIED · 실제 schema: marketplace ·  MIG-REVERSAL: partial reversals use owner lock and cumulative absolute sum <= original; source_key remains unique. No direct updates/deletes.
...
| reversal_of_entry_id | uuid | 허용 | FK | reversal_of_entry_id uuid |
...
상태: MODIFIED · 실제 schema: marketplace ·  MIG-NET/SET-02; API netAmount = gross_net_amount = credit - debit. Keep legacy net_amount = gross_net_amount - hold; payout bounded by post-hold net.
...
| gross_net_amount | bigint | 불가 |  | PLANNED gross_net_amount bigint NOT NULL |
...
상태: MODIFIED · 실제 schema: marketplace ·  SET-03; external Commerce approvals/fence, no cross-service FK.
...
| approval_id | uuid | 불가 |  | PLANNED approval_id uuid NOT NULL |
| fence_id | uuid | 불가 |  | PLANNED fence_id uuid NOT NULL |
| dispatch_permit_id | uuid | 허용 |  | PLANNED dispatch_permit_id uuid NULL |
| operation_hash | text | 불가 |  | PLANNED operation_hash text NOT NULL |
...
| next_retry_at | timestamptz | 허용 |  | PLANNED next_retry_at timestamptz NULL |
...
## seller_recognition_unit

상태: PLANNED · 실제 schema: marketplace · SET-01; snapshot/projection, no FK to Commerce.

| 컬럼 | 타입 | NULL | 키 | SQL 정의 / 계획 |
| --- | --- | --- | --- | --- |
| recognition_id | uuid | 불가 | PK | PLANNED recognition_id uuid NOT NULL |
| seller_id | uuid | 불가 |  | PLANNED seller_id uuid NOT NULL |
| order_id | uuid | 불가 |  | PLANNED order_id uuid NOT NULL |
| source_unit_or_charge_id | text | 불가 |  | PLANNED source_unit_or_charge_id text NOT NULL |
| source_event_id | uuid | 불가 |  | PLANNED source_event_id uuid NOT NULL |
| recognized_amount | bigint | 불가 |  | PLANNED recognized_amount bigint NOT NULL |
| commission_amount | bigint | 불가 |  | PLANNED commission_amount bigint NOT NULL |
| reversed_amount | bigint | 불가 |  | PLANNED reversed_amount bigint NOT NULL |
| reversed_fee_amount | bigint | 불가 |  | PLANNED reversed_fee_amount bigint NOT NULL |
...
## seller_carry_forward

상태: PLANNED · 실제 schema: marketplace · SET-02; signed debt/credit, no duplicate allocation.

| 컬럼 | 타입 | NULL | 키 | SQL 정의 / 계획 |
| --- | --- | --- | --- | --- |
| carry_forward_id | uuid | 불가 | PK | PLANNED carry_forward_id uuid NOT NULL |
| seller_id | uuid | 불가 |  | PLANNED seller_id uuid NOT NULL |
| currency | text | 불가 |  | PLANNED currency text NOT NULL |
| source_settlement_id | uuid | 불가 | FK, UK | PLANNED source_settlement_id uuid NOT NULL |
| signed_amount | bigint | 불가 |  | PLANNED signed_amount bigint NOT NULL |
| target_settlement_id | uuid | 허용 | FK | PLANNED target_settlement_id uuid NULL |
| source_ledger_ids | jsonb | 불가 |  | PLANNED source_ledger_ids jsonb NOT NULL |
| status | text | 불가 |  | PLANNED status text NOT NULL |
...
- FK seller_carry_forward_source_settlement_id_planned_fk: (source_settlement_id) → settlement.settlement(settlement_id)
- FK seller_carry_forward_target_settlement_id_planned_fk: (target_settlement_id) → settlement.settlement(settlement_id)
...
## simulated_bank_receipt

상태: PLANNED · 실제 schema: marketplace · SET-03; separate adapter transaction, no FK tying bank commit to caller commit.

| 컬럼 | 타입 | NULL | 키 | SQL 정의 / 계획 |
| --- | --- | --- | --- | --- |
| bank_receipt_id | uuid | 불가 | PK | PLANNED bank_receipt_id uuid NOT NULL |
| idempotency_key | text | 불가 | UK | PLANNED idempotency_key text NOT NULL |
| dispatch_permit_id | uuid | 불가 |  | PLANNED dispatch_permit_id uuid NOT NULL |
| operation_hash | text | 불가 |  | PLANNED operation_hash text NOT NULL |
...
상태: PLANNED · 실제 schema: marketplace · MIG-BANK/SET-04; bank payout reconciliation only; fixed snapshot cursor and immutable run inputs.
...
## bank_reconciliation_receipt

상태: PLANNED · 실제 schema: marketplace · MIG-BANK/SET-04; immutable observed bank fact. run_id is FIRST observation; later runs compare without updating original receipt. No FK to internal attempt: external-only receipts must be representable.
...
- FK bank_reconciliation_receipt_run_id_planned_fk: (run_id) → settlement.bank_reconciliation_run(run_id)
...
상태: PLANNED · 실제 schema: marketplace · MIG-BANK/SET-04; deterministic identity excludes run ID, repeat observation reuses discrepancy; approval is external Commerce ID. Legacy reconciliation_* remain read-only.
...
| bank_receipt_id | uuid | 허용 | FK | PLANNED bank_receipt_id uuid NULL |
| payout_attempt_id | uuid | 허용 | FK | PLANNED payout_attempt_id uuid NULL |
...
| approval_id | uuid | 허용 |  | PLANNED approval_id uuid NULL |
| resolution_event | jsonb | 허용 |  | PLANNED resolution_event jsonb NULL |
| recorded_at | timestamptz | 불가 |  | PLANNED recorded_at timestamptz NOT NULL |
...
- FK bank_reconciliation_discrepancy_run_id_planned_fk: (run_id) → settlement.bank_reconciliation_run(run_id)
- FK bank_reconciliation_discrepancy_bank_receipt_id_planned_fk: (bank_receipt_id) → settlement.bank_reconciliation_receipt(bank_receipt_id)
- FK bank_reconciliation_discrepancy_payout_attempt_id_planned_fk: (payout_attempt_id) → settlement.settlement_payout_attempt(payout_attempt_id)
