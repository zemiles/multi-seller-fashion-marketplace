# discovery-data-service — v1 목표 / 미적용 데이터 사전
...
상태: MODIFIED · 실제 schema: marketplace ·  DIS-02; externally owned product/revision IDs; no Commerce DB read.
...
| product_id | uuid | 불가 |  | PLANNED product_id uuid NOT NULL |
| simulated | boolean | 불가 |  | PLANNED simulated boolean NOT NULL |
...
## search_projection_generation

상태: PLANNED · 실제 schema: marketplace · DIS-01; partial unique status ACTIVE; verified atomic generation switch.

| 컬럼 | 타입 | NULL | 키 | SQL 정의 / 계획 |
| --- | --- | --- | --- | --- |
| generation_id | uuid | 불가 | PK | PLANNED generation_id uuid NOT NULL |
| status | text | 불가 |  | PLANNED status text NOT NULL |
| snapshot_id | uuid | 불가 |  | PLANNED snapshot_id uuid NOT NULL |
| watermarks | jsonb | 불가 |  | PLANNED watermarks jsonb NOT NULL |
| created_at | timestamptz | 불가 |  | PLANNED created_at timestamptz NOT NULL |
| activated_at | timestamptz | 허용 |  | PLANNED activated_at timestamptz NULL |
...
CREATE UNIQUE INDEX uq_search_generation_active_v1 ON marketplace.search_projection_generation (status) WHERE status = 'ACTIVE';
```

## search_product_projection

상태: PLANNED · 실제 schema: marketplace · DIS-01; all product/seller/revision IDs external, not FKs; query indexes on category/price/time+product_id.

| 컬럼 | 타입 | NULL | 키 | SQL 정의 / 계획 |
| --- | --- | --- | --- | --- |
| generation_id | uuid | 불가 | PK, FK | PLANNED generation_id uuid NOT NULL |
| product_id | uuid | 불가 | PK | PLANNED product_id uuid NOT NULL |
| revision_id | uuid | 불가 |  | PLANNED revision_id uuid NOT NULL |
| source_version | bigint | 불가 |  | PLANNED source_version bigint NOT NULL |
| seller_id | uuid | 불가 |  | PLANNED seller_id uuid NOT NULL |
| title | text | 불가 |  | PLANNED title text NOT NULL |
| category_id | uuid | 불가 |  | PLANNED category_id uuid NOT NULL |
| brand_id | uuid | 허용 |  | PLANNED brand_id uuid NULL |
| min_price | bigint | 불가 |  | PLANNED min_price bigint NOT NULL |
| max_price | bigint | 불가 |  | PLANNED max_price bigint NOT NULL |
| available | boolean | 불가 |  | PLANNED available boolean NOT NULL |
| public_snapshot | jsonb | 불가 |  | PLANNED public_snapshot jsonb NOT NULL |
| published_at | timestamptz | 불가 |  | PLANNED published_at timestamptz NOT NULL |

PK: (generation_id, product_id)

UNIQUE: 없음

- FK search_product_projection_generation_id_planned_fk: (generation_id) → discovery.search_projection_generation(generation_id)

```sql
CONSTRAINT search_product_projection_source_version_v1_ck CHECK (source_version >= 0)
CREATE INDEX ix_search_price_v1 ON marketplace.search_product_projection (generation_id, category_id, min_price, product_id);
CREATE INDEX ix_search_newest_v1 ON marketplace.search_product_projection (generation_id, published_at DESC, product_id);
```
