...
erDiagram
    ...
    commerceMember["commerce.member"] {
        ...
        integer session_version "required"
    }
    ...
    commerceVerificationToken["commerce.verification_token"] {
        uuid token_id PK "required"
        uuid member_id FK "required"
        text token_hash UK "required"
        text purpose "required"
        timestamptz expires_at "required"
        timestamptz used_at "nullable"
    }
    commerceSellerInvitation["commerce.seller_invitation"] {
        uuid invitation_id PK "required"
        uuid seller_id FK "required"
        uuid accepted_member_id FK "nullable"
        text email_normalized "required"
        jsonb roles "required"
        text token_hash UK "required"
        timestamptz expires_at "required"
        text status "required"
    }
    ...
    commerceMember ||..o{ commerceVerificationToken : "member_id"
    commerceSeller ||..o{ commerceSellerInvitation : "seller_id"
    commerceMember |o..o{ commerceSellerInvitation : "accepted_member_id"
```
